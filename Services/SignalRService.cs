...
public class SignalRService : ISignalRService
{
        ...
        IServiceScopeFactory serviceScopeFactory)
    {
        ...
        _notificationHubContext =
            notificationHubContext ?? throw new ArgumentNullException(nameof(notificationHubContext));
    ...
    public async Task SendCsvAnalysisProgressAsync(string connectionId, int progress, string status, string message,
    ...
    public async Task SendCsvAnalysisCompleteAsync(string connectionId, AnalysisResult result)
    {
        ...
        await SendCsvAnalysisProgressAsync(connectionId, 100, "analyzed", "Analyse terminée avec succès",
            result.Analysis);
        ...
        try
        {
            ...
            if (analysisResult.Success && analysisResult.Analysis != null)
                _logger.LogInformation(
                    $"[ProcessCsvUpload] ✅ Analyse réussie avec {analysisResult.Analysis.Actions?.Count ?? 0} actions pour connectionId: {connectionId}");
            else
                _logger.LogWarning(
                    $"[ProcessCsvUpload] ⚠️ Analyse échouée. Success: {analysisResult.Success}, Analysis null: {analysisResult.Analysis == null}");

            // Mettre à jour la progression après l'analyse
            await SendCsvAnalysisProgressAsync(connectionId, 70, "analyzing",
            ...
            if (analysisResult.Success)
            {
                ...
                if (analysisResult.TableData != null && !string.IsNullOrEmpty(connectionId))
                {
                    ...
                    FileDataStore.SetCsvData(analysisResult.TableData, connectionId);
                    _logger.LogInformation(
                        $"Données CSV brutes ({analysisResult.TableData.Count} lignes) de {fileName} stockées dans CsvDataStore pour connectionId: {connectionId}");
                }
                else
                {
                    if (analysisResult.TableData == null)
                        _logger.LogWarning(
                            $"analysisResult.TableData est null après l'analyse du fichier {fileName} pour {connectionId}. Les données ne seront pas stockées.");
                    if (string.IsNullOrEmpty(connectionId))
                        _logger.LogWarning(
                            $"ConnectionId est null ou vide après l'analyse du fichier {fileName}. Les données ne seront pas stockées.");
                }
            ...
            {
                _logger.LogError(
                    $"Échec de l'analyse du fichier {fileName} pour {connectionId}: {analysisResult.ErrorMessage}");
                await SendCsvAnalysisErrorAsync(connectionId,
                    analysisResult.ErrorMessage ?? "Une erreur inconnue est survenue lors de l'analyse.");
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                $"Erreur lors du traitement de l'upload du fichier {fileName} pour la connexion {connectionId}");
            await SendCsvAnalysisErrorAsync(connectionId,
                $"Erreur lors du traitement du fichier {fileName}: {ex.Message}");
        }
    }
    ...
    public async Task SendImportProgressAsync(string? connectionId, int current, int total, string message)
    {
        ...
        var progress = total > 0 ? (int)(current * 100.0 / total) : 0;
            ...
            switch (messageType)
            {
                ...
                case "log":
                    if (TryExtractLogEntry(typedMessage, out var logEntry))
                        ...
                        await _csvImportHubContext.Clients.Client(connectionId)
                            .SendAsync("ReceiveMessage", typedMessage);
                    break;
    ...
    private bool TryExtractLogEntry(object message, out LogEntry logEntry)
    {
        ...
        try
        {
            if (message is JsonElement jsonElement && jsonElement.ValueKind == JsonValueKind.Object)
            {
                ...
                {
                    var timestampStr = timestampProperty.GetString();
                ...
                if (string.IsNullOrEmpty(logEntry.Message)) return false;
    ...
    private LogEntry CreateLogEntry(DateTime timestamp, MsLogLevel level, string message, string? category = null,
    ...
    public async Task NotifyUnsavedChanges(string connectionId, string message,
        MsLogLevel logLevel = MsLogLevel.Information)
    {
