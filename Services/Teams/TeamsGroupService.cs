    ...
    public class TeamsGroupService : ITeamsGroupService
    {
        ...
        public TeamsGroupService(ILogger<TeamsGroupService> logger, IConfigService configService)
        {
            ...
            try
            {
                ...
                // 🔧 AMÉLIORATION SÉCURITÉ : Récupération depuis la configuration
                // Utilisation des propriétés de la configuration TeamsIntegrationConfig
                var clientId = _teamsIntegrationConfig.AzureADClientId;
                var tenantId = _teamsIntegrationConfig.AzureADTenantId;
                var clientSecret = _teamsIntegrationConfig.AzureADClientSecret;
                
                if (string.IsNullOrEmpty(clientId) || string.IsNullOrEmpty(tenantId) || string.IsNullOrEmpty(clientSecret))
                {
                    _logger.LogWarning("⚠️ Configuration Azure AD incomplète dans TeamsIntegrationConfig - Intégration Teams désactivée");
                    _isEnabled = false;
                    _isInitialized = true;
                    return;
                }
        ...
        private void ReloadConfiguration()
        {
            ...
            _logger.LogDebug("DEBUG: TeamsGroupService - Configuration rechargée. Enabled = {IsEnabled}", _teamsIntegrationConfig.Enabled);
        ...
        /// </summary>
        public async Task<string> CreateClassTeamAsync(string className, string classDescription, string classMailNickname, string teacherUserId)
        {
            if (!EnsureEnabled())
                return string.Empty;

            _logger.LogInformation("🚀 Création classe Teams: {ClassName}", className);
            
            try
            {
                // 1. Créer la classe éducation
                string classId = await CreateEducationClassAsync(className, classDescription, classMailNickname);
                ...
                _logger.LogInformation("✅ Classe et équipe créées avec succès: {ClassId}", classId);
                return classId;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "❌ Erreur création classe Teams {ClassName}", className);
                throw;
            }
        }
        ...
        public async Task<string> GetChannelIdByNameAsync(string teamId, string channelName)
        {
            ...
            try
            {
                ...
                var channel = channels.Value.FirstOrDefault(c => c.DisplayName.Equals(channelName, StringComparison.OrdinalIgnoreCase));
        ...
        public async Task CreateFolderInChannelFilesAsync(string teamId, string channelId, string folderName)
        {
            ...
            for (int attempt = 0; attempt < maxRetries; attempt++)
            {
                ...
                {
                    _logger.LogWarning(ex, "⚠️ Tentative {Attempt}/{Max} échec création dossier '{FolderName}'", attempt + 1, maxRetries, folderName);
        ...
        /// </summary>
        public async Task SetReadOnlyPermissionsForVisitorGroupAsync(string teamId, string channelId, string folderName, string visitorGroupName, string permission)
        {
            if (!EnsureEnabled())
                return;
            ...
            try
            {
                _logger.LogInformation("🔐 Configuration permissions '{Permission}' pour dossier '{FolderName}'", permission, folderName);
        ...
        public async Task<bool> TeamExistsAsync(string teamName)
        {
            ...
            try
            {
                ...
                {
                    requestConfiguration.QueryParameters.Filter = $"displayName eq '{teamName}' and resourceProvisioningOptions/any(x:x eq 'Team')";
                });
        ...
        public async Task<List<string>> GetTeamMembersAsync(string teamId)
        {
            ...
            try
            {
                ...
                var memberIds = members?.Value?.Select(m => m.Id).Where(id => !string.IsNullOrEmpty(id)).ToList() ?? new List<string>();
        ...
        private async Task<string> CreateEducationClassAsync(string className, string classDescription, string classMailNickname)
        {
        ...
        private async Task AddTeacherToClassAsync(string classId, string teacherUserId)
        {
            ...
            if (teacher == null)
            {
                throw new Exception($"L'utilisateur avec l'ID '{teacherUserId}' n'a pas été trouvé en tant que educationUser.");
            }
        ...
        private async Task CreateTeamForClassAsync(string classId)
        {
            try
            {
                ...
                string channelId = await GetChannelIdByName(groupId, "Histoire");
        ...
        public async Task SetReadOnlyPermissionsForVisitorGroup(string teamId, string channelId, string folderName, string visitorGroupName, string perm)
        {
            if (!EnsureEnabled()) return;
            ...
            try
            {
                ...
                foreach (var permission in permissions.Value)
                {
                        ...
                        permission.GrantedToV2.SiteGroup.DisplayName.Contains(visitorGroupName))
                    {
                        ...
                        _logger.LogInformation("✅ Groupe trouvé: ID = {SiteGroupId}, Nom = {SiteGroupName}", siteGroupId, siteGroupName);
                        break;
                    }
                }
        ...
        public async Task SetFolderPermissionsForSiteMembers(string driveId, string folderId, string siteGroupId, string siteGroupName, string permissionType = "read")
        {
            if (!EnsureEnabled()) return;
            ...
            try
            {
                ...
                else
                {
                    ...
                    _logger.LogInformation("✅ Permissions '{PermissionType}' créées pour dossier '{FolderId}'", permissionType, folderId);
                }
            }
            catch (ServiceException ex)
            {
