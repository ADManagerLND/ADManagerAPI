...
public interface ISpreadsheetImportService
{
    ...
    Task<AnalysisResult> AnalyzeSpreadsheetDataAsync(List<Dictionary<string, string>> spreadsheetData,
        ImportConfig config, string? connectionId = null, CancellationToken cancellationToken = default);

    Task<ImportResult> ExecuteImportFromActionsAsync(List<Dictionary<string, string>> spreadsheetData,
        ImportConfig config, List<LegacyImportActionItem> actions, string? connectionId = null);

    Task<ImportResult> ExecuteImportFromAnalysisAsync(ImportAnalysis analysis, ImportConfig config,
