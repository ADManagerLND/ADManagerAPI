...
public partial class SpreadsheetImportService
{
    ...
    {
        _logger.LogTrace(
            $"[OU_DEBUG_BUILD] BuildOuPath appelé avec ouValueFromCsv: '{ouValueFromCsv}', defaultOu: '{defaultOu}'");
        var cleanDefaultOu = defaultOu?.Trim();
        ...
        var isLikelyDn = ouValueFromCsv.Contains("DC=", StringComparison.OrdinalIgnoreCase) ||
                         (ouValueFromCsv.Contains("OU=", StringComparison.OrdinalIgnoreCase) &&
                          ouValueFromCsv.Contains(","));

        if (isLikelyDn)
        {
            _logger.LogDebug(
                $"[OU_DEBUG_BUILD] ouValueFromCsv '{ouValueFromCsv}' semble être un DN. Extraction des composants OU et DC.");
            ...
            if (components.Any())
            {
                var extractedOuPath = string.Join(",", components);
                _logger.LogDebug($"[OU_DEBUG_BUILD] Chemin d'OU extrait du DN: '{extractedOuPath}'");
                return extractedOuPath;
            }

            _logger.LogWarning(
                $"[OU_DEBUG_BUILD] Aucun composant OU ou DC trouvé dans le DN présumé '{ouValueFromCsv}'. Retour de defaultOu.");
            return cleanDefaultOu;
        }
        ...
        var formattedOuPathRelative = string.Join(",", ouParts);

        if (string.IsNullOrEmpty(formattedOuPathRelative))
        {
            _logger.LogWarning(
                $"[OU_DEBUG_BUILD] ouValueFromCsv '{ouValueFromCsv}' n'a pas produit de parties d'OU valides après parsing. Retour de defaultOu.");
            return cleanDefaultOu;
        }
        ...
        {
            _logger.LogDebug(
                $"[OU_DEBUG_BUILD] defaultOu est vide. Retour de l'OU formatée relative: '{formattedOuPathRelative}'");
            return formattedOuPathRelative;
        }

        var finalPath = $"{formattedOuPathRelative},{cleanDefaultOu}";
