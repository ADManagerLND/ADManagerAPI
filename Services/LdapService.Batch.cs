...
public partial class LdapService
{
    ...
    public async Task<List<UserModel>> GetUsersBatchAsync(List<string> samAccountNames)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < samAccountNames.Count; i += batchSize)
            {
            ...
            _logger.LogInformation(
                $"✅ Chargement batch: {results.Count} utilisateurs trouvés sur {samAccountNames.Count} recherchés");
            return results;
        }
        catch (Exception ex)
        {
    ...
    private async Task<List<UserModel>> GetUsersBatchInternalAsync(List<string> samAccountNames)
    {
        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                // ✅ CORRECTION CRITIQUE : Récupérer TOUS les attributs au lieu d'une liste fixe
                var req = new SearchRequest(_baseDn, combinedFilter, SearchScope.Subtree, "*");
                ...
                _logger.LogDebug(
                    $"Batch interne: {results.Count} utilisateurs extraits pour {samAccountNames.Count} demandés");
                return results;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    $"Erreur dans GetUsersBatchInternalAsync pour {samAccountNames.Count} utilisateurs");
                return results;
            }
        });
    }
    ...
    public async Task<List<UserModel>> SearchUsersAsync(string searchBase, string ldapFilter)
    {
        ...
        try
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    var req = new SearchRequest(searchBase, ldapFilter, SearchScope.Subtree, "sAMAccountName",
                        "displayName", "distinguishedName", "givenName", "sn", "mail", "userPrincipalName");
                    ...
                    _logger.LogInformation(
                        $"🔍 Recherche LDAP avec filtre '{ldapFilter}': {results.Count} utilisateurs trouvés");
                    return results;
                }
                catch (Exception ex)
                {
        ...
        try
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    var combinedFilter =
                        $"(&(objectClass=user)(objectCategory=person)(|{string.Join("", userFilters)}))";
                    ...
                    var req = new SearchRequest(_baseDn, combinedFilter, SearchScope.Subtree,
                        attributesToLoad.Distinct().ToArray());
                    req.SizeLimit = 2000;
                    ...
                    foreach (SearchResultEntry entry in res.Entries)
                        try
                        {
                            var samAccountName = entry.Attributes.Contains("sAMAccountName") &&
                                                 entry.Attributes["sAMAccountName"].Count > 0
                    ...
                    _logger.LogInformation(
                        $"📋 Attributs batch: {results.Count} utilisateurs traités pour {attributeNames.Count} attributs");
                    return results;
                }
                catch (Exception ex)
                {
    ...
    public async Task<HashSet<string>> GetOrganizationalUnitsBatchAsync(List<string> ouPaths)
    {
        ...
        try
        {
            ...
            for (var i = 0; i < ouPaths.Count; i += batchSize)
            {
            ...
            _logger.LogInformation(
                $"✅ Vérification batch OUs: {existingOus.Count} OUs existantes sur {ouPaths.Count} vérifiées");
            return existingOus;
        }
        catch (Exception ex)
        {
    ...
    private async Task<List<string>> GetOrganizationalUnitsBatchInternalAsync(List<string> ouPaths)
    {
        ...
        return await Task.Run(() =>
        {
            try
            {
                ...
                var req = new SearchRequest(_baseDn, combinedFilter, SearchScope.Subtree, "distinguishedName");
                req.SizeLimit = 1000;
                ...
                foreach (SearchResultEntry entry in res.Entries)
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(dn)) existingOus.Add(dn);
                    }
                    catch (Exception ex)
                    {
    ...
    private UserModel? CreateUserModelFromSearchResult(SearchResultEntry entry)
    {
        ...
        additionalAttributes["distinguishedName"] = distinguishedName;
