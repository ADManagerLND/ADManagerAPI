...
public static class AnalysisDataStore
{
    ...
    public static void SetAnalysis(string connectionId, ImportAnalysis analysis)
    {
        ...
        Console.WriteLine(
            $"[AnalysisDataStore] Analyse stockée pour connectionId '{connectionId}'. Actions: {analysis?.Actions?.Count ?? 0}");
    }
    ...
    public static ImportAnalysis? GetAnalysis(string connectionId)
    {
        ...
        Console.WriteLine(
            $"[AnalysisDataStore] Récupération analyse pour connectionId '{connectionId}'. Trouvée: {analysis != null}. Actions: {analysis?.Actions?.Count ?? 0}");
        return analysis;
    }
    ...
    public static void ClearAnalysis(string connectionId)
    {
        ...
        var removed = _analysisData.TryRemove(connectionId, out _);
        Console.WriteLine(
            $"[AnalysisDataStore] Suppression analyse pour connectionId '{connectionId}'. Supprimée: {removed}");
    }

    public static void SetLatestAnalysis(ImportAnalysis analysis)
    {
        Console.WriteLine(
            "[AnalysisDataStore] ATTENTION: Utilisation de SetLatestAnalysis (legacy). Recommandé d'utiliser SetAnalysis avec connectionId.");
    ...
    {
        Console.WriteLine(
            "[AnalysisDataStore] ATTENTION: Utilisation de GetLatestAnalysis (legacy). Recommandé d'utiliser GetAnalysis avec connectionId.");
