    ...
    public class ConfigController : ControllerBase
    {
        ...
        public async Task<ActionResult<SavedImportConfig>> GetADMappingConfiguration(string configId)
        {
            try
            {
                ...
                {
                    return NotFound($"Configuration de mapping AD avec l'ID {configId} non trouvée");
                }
                return Ok(configuration);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erreur lors de la récupération de la configuration de mapping AD {ConfigId}", configId);
        ...
        public async Task<ActionResult<SavedImportConfig>> SaveADMappingConfiguration([FromBody] SavedImportConfig configuration)
        {
            try
            {
        ...
        public async Task<ActionResult<SavedImportConfig>> UpdateADMappingConfiguration(string configId, [FromBody] SavedImportConfig configuration)
        {
            try
            {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la mise à jour de la configuration de mapping AD {ConfigId}", configId);
        ...
        public async Task<ActionResult> DeleteADMappingConfiguration(string configId)
        {
            ...
            {
                _logger.LogError(ex, "Erreur lors de la suppression de la configuration de mapping AD {ConfigId}", configId);
        ...
        public async Task<ActionResult<ValidationResult>> ValidateHeaderMapping([FromBody] Dictionary<string, string> headerMapping)
        {
            try
            {
                var attributes = await _configService.GetUserAttributesAsync();
        ...
        public ActionResult<List<MappingDisplayItem>> ConvertToDisplayFormat([FromBody] Dictionary<string, string> headerMapping)
        {
            try
            {
        ...
        public ActionResult<List<PredefinedColumnDefinition>> GetPredefinedColumns()
        {
            try
            {
                ...
                {
                    // Identité
                    new PredefinedColumnDefinition
                    {
                        Key = "prenom",
                        Label = "Prénom",
            ...
            [FromBody] BasicMappingValidationRequest request)
        {
            try
            {
                ...
                if (request.Template.Contains("%"))
                {
                    ...
                    {
                        errors.Add(syntaxValidation.Error ?? "Syntaxe de template invalide");
                    }
                }
        ...
        private (bool IsValid, string? Error) ValidateTemplateSyntax(string template)
        {
            try
            {
                ...
                if (percentCount % 2 != 0)
                {
                    return (false, "Nombre impair de caractères % dans le template");
                }
